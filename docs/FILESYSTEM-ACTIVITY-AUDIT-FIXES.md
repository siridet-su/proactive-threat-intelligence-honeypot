...
**In progress:** `FA-001` — align audit summary and search state (follow-up remediation on commit `93715e3`).

## Remediation backlog

| ID | Priority | Status | Original items | Problem | Acceptance criteria | Evidence |
| --- | --- | --- | --- | --- | --- | --- |
| `FA-001` | `P0` | `IN PROGRESS` | `FS-001`, `FS-005`, `FS-007` | Audit filter options, `0/N`, totals, and path counts are calculated from the live snapshot plus only 12 recent closed sessions. Remote results remain local to the selector. | Audit totals and filtered results include the complete retained closed-session directory; remote pages become part of one authoritative parent data model; selected/pinned semantics and path options agree with the result set. | Follow-up remediation on commit `93715e3`: preserved selected session across search queries by strictly isolating search results to selector dropdown options without replacing authoritativeClosedSessions, allSessions, or sessionById; introduced stable canonical scope key `createAuditScopeKey({ hideHome, targetPath, q })` with generation counters and AbortControllers to discard stale out-of-order summaries and searches; aligned MongoDB `$facet` aggregation with `normalizeSessionAuditSummary` semantics (closed-only filtering, legacy `session_id`, `cwdState.path` search, visited paths with `cwdState.path`, `fromPath`, non-failed `toPath`, root-only non-home behavior, and zero in-memory heap accumulation); bound directory pagination scope across pages; debounced search input (250ms); added 6 regression test suites A-F (190 passing tests, 0 ESLint warnings/errors, Next.js build clean, clean git diff --check). Kept `IN PROGRESS` with recorded dependency on `FA-002`. |
...
| --- | --- | --- |
| 2026-09-16 | Follow-up FA-001: aligned audit summary and search state, isolated search dropdown options, bound canonical query scope, debounced input, and added regression test suites A-F. | 10 test files / 190 tests passing; clean ESLint (0 errors, 0 warnings); clean Next.js build; clean git diff --check. |
