...
| `FA-005` | `P0` | `IN PROGRESS` | `FS-006` | A shared `hop` URL is checked only against the first 80 history events and then discarded. | A deep link resolves a retained hop on any page, either through direct event lookup or bounded cursor traversal; invalid/expired hop IDs produce an explicit state; request cancellation and maximum work are defined. | Finalized remediation closing deep-hop lifecycle and replay gaps: (1) preserved deep-hop scope through remote retained-session lookup via RemoteAuditLookupManager with generation guards, single-flight execution, targetHopId tracking, and abort on user session navigation; (2) preserved terminal hop-resolution state across background history refreshes via SessionHopLifecycleManager re-emitting authoritative state without resetting to resolving; (3) restored truthful mixed-schema hop numbering via migration-compatible $or filter matching normal history pagination, removed broad catch/fan-out fallback, and documented strict database operation bounds (canonical success: 2, cross-session: 1, legacy success: 3-4, unknown: 3, overlength: 0); (4) replaced simulation tests with production behavior tests invoking RemoteAuditLookupManager, SessionHopLifecycleManager, computeNextReplayEventId, computeTogglePlayState, and renderToStaticMarkup of CwdRouteHistory; 14 test files / 264 tests passing, clean ESLint (0 errors, 0 warnings), clean Next.js build, clean git diff --check. |
