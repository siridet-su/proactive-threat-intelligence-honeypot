...
**In progress:** `FA-004` — separate action status from capability health.
...
| `FA-004` | `P0` | `IN PROGRESS` | `FS-011` | Status polling can expose a false capability error when the 10-second Pi-health cache expires, and each poll still reads action and session state separately. | Pending-action status remains truthful after cache expiry without producing Pi health ping storms; database work per poll is documented and minimized; tests use fake timers to assert request cadence, total requests, timeout, abort, and terminal behavior. | Separated status query from capability probing in `/api/sessions/[id]/actions/terminate` when `actionId` query parameter is present: status-only polls omit `available` without calling Pi `/v1/health` or reading `responseControlCachedHealth`, preventing pending actions from converting to false capability errors upon 10-second health cache expiry; capability probe without `actionId` retains 10s anti-ping-storm cache via `responseControlHealthy()`; `terminateCapabilityFrom` and `ResponseActionPollingController` preserve last authoritative capability state when `available` is omitted; single MongoDB aggregation pipeline with `$lookup` to `session_state` in `getTerminateActionWithState` performs 1 read round trip for ordinary pending polls (0 writes while pending), and 1 atomic `findOneAndUpdate` write only upon reconciliation to terminal `verified` (matching `closedAt >= requestedAt`) or `failed` (`verification_timeout` after 20s); concurrent reconciliation remains idempotent; database operations documented in JSDoc; 9 regression tests in `response-action-status.test.ts` plus 24 controller/lifecycle manager tests in `response-action-poller.test.ts` (13 test files / 248 tests passing, clean ESLint with 0 errors/0 warnings, clean Next.js production build, clean git diff --check). Outstanding integration gate: manual response-action smoke test with live response agent. |
...
| --- | --- | --- |
| 2026-09-17 | Remediated FA-004 (in progress): separated action status from capability health, omitted available from status responses to prevent false capability errors on cache expiry, preserved client capability, minimized MongoDB work to 1 read / 0 writes while pending, and added regression tests. | 13 test files / 248 tests passing; clean ESLint (0 errors, 0 warnings); clean Next.js build; clean git diff --check. |
