...
**In progress:** `FA-003` — stabilize response action polling.
...
| `FA-003` | `P0` | `IN PROGRESS` | `FS-011` | Response-action polling restarts its effect whenever a new action object is stored, resetting delay and the client deadline. A pending non-live action can poll near 100 ms intervals. | A single polling lifecycle survives state updates; delay increases monotonically to the configured ceiling; polling stops at terminal state or the bounded deadline; changing selected session aborts the prior lifecycle; terminal feedback remains visible. | Standalone `ResponseActionPollingController` establishes a single polling lifecycle keyed by `sessionId` and `actionId`, decoupling polling execution from React rerenders and `visibleTerminateAction` object identity changes (`requested` -> `delivered`); absolute deadline derived from `action.requestedAt + 24_000` (falling back to `now + 24_000`); monotonic delay progression (initial 100 ms non-live, 1,000 ms live, 1.5x factor) capped strictly at 3,500 ms (`100 -> 150 -> 225 -> 338 -> 507 -> 761 -> 1142 -> 1713 -> 2570 -> 3500`); request overlap prevented by waiting for completion; network errors continue backoff without resetting; terminal outcomes (`verified`, `failed`, or `timeout`) emit persistent toast feedback exactly once; abort on unmount, session switch, or action change cleanly halts in-flight requests and suppresses stale updates; 13 fake-timer tests in `response-action-poller.test.ts` asserting exact cadence, deadline derivation, and 13-request maximum bound over 24s (228 tests passing across 12 files, clean ESLint, clean build, clean git diff --check). |
...
| --- | --- | --- |
| 2026-09-16 | Remediated FA-003 (in progress): stabilized response action polling with single-lifecycle controller, monotonic backoff capped at 3.5s, absolute deadline derivation, and exactly-once terminal feedback. | 12 test files / 228 tests passing; clean ESLint (0 errors, 0 warnings); clean Next.js build; clean git diff --check. |
