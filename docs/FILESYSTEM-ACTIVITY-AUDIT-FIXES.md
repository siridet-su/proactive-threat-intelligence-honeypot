...
**In progress:** `FA-001` — make Audit directory filtering and counts authoritative (follow-up remediation on commit `64b85ef`).

## Remediation backlog

| ID | Priority | Status | Original items | Problem | Acceptance criteria | Evidence |
| --- | --- | --- | --- | --- | --- | --- |
| `FA-001` | `P0` | `IN PROGRESS` | `FS-001`, `FS-005`, `FS-007` | Audit filter options, `0/N`, totals, and path counts are calculated from the live snapshot plus only 12 recent closed sessions. Remote results remain local to the selector. | Audit totals and filtered results include the complete retained closed-session directory; remote pages become part of one authoritative parent data model; selected/pinned semantics and path options agree with the result set. | Follow-up remediation completed for commit `64b85ef`: added authoritative summary facets (`/api/filesystem-topology/audit-summary` and `audit-sessions?summary=1`), decoupled directory vs search pagination state in `createAuditDirectoryStore`, scoped remote search queries to active filters, mutually exclusive pagination/completed rendering via `getPaginationRenderState`, initial request error/retry lifecycle, and 15 regression tests in `filesystem-audit-directory.test.ts` (179 passing tests, 0 lint warnings, clean build). Kept `IN PROGRESS` with recorded dependency on `FA-002` for server-side MongoDB pre-pagination filtering. |
