...
| `FA-016` | `P1` | `IN PROGRESS` | `FS-007` | Full-scan summary aggregation and cursor skip on 1,878+ documents, plus projection lifecycle/convergence gaps found during re-audit | Exact projection semantics, bounded item plans, truthful count/summary bounds, safe migration convergence, retention, and isolated MongoDB evidence | Follow-up preserves v2 migration, adds generation-owned readiness/CAS and partial-indexed reconciliation, exact `$unionWith` overflow pagination across the full retained population, canonical padded-ID convergence, and deterministic crash/race fixtures. The isolated integration passes 12 dashboard tests and six processor FA-016 integration tests; evidence is recorded in [`docs/validation/FA-016-audit-scale.md`](validation/FA-016-audit-scale.md). FA-016 remains IN PROGRESS pending final validation/re-audit; FS-007 remains PARTIAL. |
