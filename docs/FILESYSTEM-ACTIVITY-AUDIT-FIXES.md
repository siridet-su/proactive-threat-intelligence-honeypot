...
| `FA-016` | `P1` | `IN PROGRESS` | `FS-007` | Full-scan summary aggregation and cursor skip on 1,878+ documents, plus projection lifecycle/convergence gaps found during re-audit | Exact projection semantics, bounded item plans, truthful count/summary bounds, safe migration convergence, retention, and isolated MongoDB evidence | Follow-up preserves v2 migration, adds event-level durable reconciliation outbox work, generation-owned readiness/CAS, separate indexed old-writer cutover probes, exact `$unionWith` overflow pagination, canonical padded-ID convergence, and deterministic crash/race fixtures. The isolated integration passes 12 dashboard tests and executes 9 production FA-016 Mongo integration tests plus 2 loopback-target safety tests; evidence is recorded in [`docs/validation/FA-016-audit-scale.md`](validation/FA-016-audit-scale.md). FA-016 remains IN PROGRESS pending final validation/re-audit; FS-007 remains PARTIAL. |
