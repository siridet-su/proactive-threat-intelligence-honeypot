...
| `FA-005` | `P0` | `IN PROGRESS` | `FS-006` | A shared `hop` URL is checked only against the first 80 history events and then discarded. | A deep link resolves a retained hop on any page, either through direct event lookup or bounded cursor traversal; invalid/expired hop IDs produce an explicit state; request cancellation and maximum work are defined. | Follow-up remediation closing deep-hop replay gaps: (1) one-shot resolution intent cleared on page-one match or direct resolution (`requestedHopRef.current = null; setRequestedHop(null)`), while `selectedHistoryEventId` maintains `?hop=<id>` in URL; switching sessions clears prior hop intent and selection; (2) modeled deep targets outside contiguous loaded window as `anchoredHop` without splicing into `history`, preserving strict contiguous newest-first sorting; Prev/Next/Play and auto-play disabled across unloaded gaps with explicit callout divider and partial time metrics (`Gap`/`Partial`); appending earlier pages reconciles `anchoredHop` seamlessly without changing selection or absolute hop number; (3) recovery banner with Clear hop / View all sessions rendered above history independently of `history.length === 0`; (4) server bounds reject identifiers >300 chars with HTTP 400 at route and 0 DB operations in server helper; single-field indexed scope `{ [sessionField]: sanitizedSessionId }` without unindexed `$or`; consolidated count work into single `$facet` aggregation pipeline with documented 2-operation canonical contract; 13 regression tests in `filesystem-hop-resolution.test.ts` (14 test files / 265 tests passing, clean ESLint 0 errors/0 warnings, clean Next.js production build, clean git diff --check). |
