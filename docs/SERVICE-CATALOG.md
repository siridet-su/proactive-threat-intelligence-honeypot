...
| Retained data backup worker | outbound B2 from Pi | Current, hardware target active; other targets prepared | current project | MongoDB retention sources → gzip archive → private Backblaze B2 | `hardware_metrics_1m` is active. `threat_events` and `filesystem_audit` require explicit Pi target activation and scoped key/policy review; the worker has no `deleteFiles` capability. |
