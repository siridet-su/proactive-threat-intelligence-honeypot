...
| Retained data backup worker | outbound B2 from Pi | Current, hardware and filesystem targets active | current project | MongoDB retention sources → gzip archive → private Backblaze B2 | `hardware_metrics_1m` and `filesystem_audit` are active on the Pi. `threat_events` remains disabled pending sensitive-data policy review; the upload worker has no `deleteFiles` capability. |
