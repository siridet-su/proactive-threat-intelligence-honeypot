...
| [Decoy-stack telemetry](decoy-stack-telemetry.md) | Target | Add dashboard/API access for current web login telemetry and later integrate stopped Docker decoys. |
| [Web-corp login telemetry](web-login-telemetry.md) | Current | Admin-only event path for fake ERP login POSTs only, with credential retention in MongoDB, raw Redis input, and SQLi indicators; page/scan logging is off. |
