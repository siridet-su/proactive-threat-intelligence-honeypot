...
| `FS-011` | `DONE` | Reduce response-action polling cost. | Pending actions do not perform a Pi health check plus multiple MongoDB reads every second; status propagation has bounded backoff or an event stream and preserves terminal-state feedback. | In-memory TTL caching (10s) on `responseControlHealthy` bypasses outbound Pi health pings on status queries; single-pass `getTerminateActionWithState` eliminates redundant MongoDB reads across collections; `CwdRouteHistory` implements exponential bounded backoff (1s–3.5s, 24s ceiling) with immediate 100ms trigger on live SSE `sessionIsLive` transition; verified and failed terminal toasts preserved; 70 Vitest tests, ESLint, and Next production build passed. |
