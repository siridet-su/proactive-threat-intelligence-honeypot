...
class WebCorpTests(TestCase):
    ...
    def test_http_page_and_form_attempt_share_server_issued_session(self):
        ...
        login = self.client.post(
            "/web/login",
            data={"login": "<script>alert(1)</script>", "password": "' OR 'a'='a' --"},
        )
        self.assertEqual(login.status_code, 200)
        events = [json.loads(path.read_text(encoding="utf-8"))
                  for path in Path(self.spool.name).glob("*.jsonl")]
        self.assertEqual(len(events), 2)
        http_event = next(item for item in events if item["event"] == "web_http_request")
        login_event = next(item for item in events if item["event"] == "web_login_attempt")
    ...
    def test_forged_and_expired_browser_session_rotate(self):
        ...
        events = [json.loads(path.read_text(encoding="utf-8"))
                  for path in Path(self.spool.name).glob("*.jsonl")]
        original = events[0]["web_session_id"]
