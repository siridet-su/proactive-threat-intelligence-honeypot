...
Status: implementation plan approved by repository inspection, 2026-08-08
...
transaction. This transaction is the only safe insertion point: when the
feature is enabled, it will also enqueue an AI-advisory outbox row that contains
only stable report references. The AI worker will run later and will load the
already persisted report. It cannot delay, change, or roll back canonical
analysis.
...
- SecureBERT trust gates and the advisory Transformer/VOMM prediction path;
- report/artifact identities, alerts, webhooks, and response safety policy.

Repository search found no hosted generative-AI client, provider/model
...
separate, explicitly configured implementation task.
...
compatible. The analysis worker passes the feature flag. Enqueue and canonical
report persistence are atomic. The default is disabled, so existing databases
gain empty additive tables and existing runtime behaviour is unchanged.
...
- bounded batch, poll, lease, timeout, retry, request/response-size, token-budget,
  and 30-day advisory-retention settings;
- `ai_advisory_fixture_response_path`, accepted only by the offline fixture
  adapter used in tests/evaluation.

The common configuration records only the owner-only secret-file path. It
validates that an enabled hosted-provider secret is a non-symlink regular file
with no group/other permissions, without retaining the secret value. A real
provider adapter must read the file at call time, must never log its contents,
and must fail closed if ownership/mode checks or provider/model configuration
fail. No service unit or production secret is added now because no provider is
present and deployment is outside this task.
...
- `schema_version`, `assessment_id`, `evidence_sha256`, `projection_sha256`;
...
It deliberately excludes raw commands, command fragments, usernames, source
...
The worker uses bounded durable claims and heartbeat renewal. Provider timeout,
...
An additive loopback-monitor endpoint returns the separate advisory record for
one report/session. Existing public session/report APIs remain unchanged. The
...
2. **Durability.** Add the rollback-compatible extension, atomic optional outbox enqueue, advisory
   storage, cache, and queue operations. Acceptance: extension/integrity tests
...
A real hosted provider adapter, provisioned secret, provider-specific structured
output API, systemd unit, deployment manifest change, live API call, analyst
study, and production deployment do not currently exist and are not fabricated
by this implementation. They require an explicitly selected provider/model and
deployment approval after the offline safety gates pass.
