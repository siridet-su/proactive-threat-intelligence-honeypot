...
Privacy begins before canonical persistence. Cowrie's configured output path and the forwarder apply the active `cowrie_output_privacy_policy.v2` policy (`cowrie_field_privacy_admin_command_evidence`, version `2.0.0`). Structured authentication credential fields and non-command text remain redacted. The exact top-level `input` field is retained only for the three reviewed Cowrie command-input event IDs because the Admin session view is authorized to inspect what was typed; it is sensitive evidence and may itself contain passwords or tokens. Public session APIs, browser print/export, PDFs, STIX, webhooks, logs, and prediction snapshots must continue to redact/exclude it. Command-input retention in Cowrie JSON, spool/quarantine, canonical event storage, and backups must follow restricted access and retention handling. Previously redacted command text is not recoverable. Where credential linkage is needed, the worker uses keyed/HMAC aliases under a separately loaded keyring rather than treating raw secrets as analytics fields.
...
|---|---|---|---|---|
| `configs/cowrie_output_privacy.v1.json` | `cowrie_field_privacy_admin_command_evidence` / `2.0.0` | Sensor/worker privacy loaders; forwarder and session runtime | Structured authentication fields/non-command text are redacted; exact Cowrie command input is retained as sensitive Admin-only evidence | Invalid policy prevents safe runtime use. Public APIs and reports retain independent redaction. Authoritative for privacy, not classification. |
