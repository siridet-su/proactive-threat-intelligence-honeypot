...
RECEIPT_PATH = PROJECT_ROOT / "evaluation" / "final_f_phase6_api_ui_receipt.v1.json"


def test_phase6_receipt_binds_implementation_and_content() -> None:
    receipt = json.loads(RECEIPT_PATH.read_text(encoding="utf-8"))
    assert receipt["schema_version"] == "final_f_phase6_api_ui_receipt.v1"
    assert receipt["status"] == "COMPLETE_VALID"
    assert receipt["phase"] == 6
