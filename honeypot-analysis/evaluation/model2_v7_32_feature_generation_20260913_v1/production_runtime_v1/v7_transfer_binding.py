...
def select_episode_tuples(
    packets: Iterable[Any], low: float, high: float
) -> list[dict[str, Any]]:
    """Select every eligible HTTP flow in the fixed episode window.

    Selection is intentionally independent of Cowrie ``file_download``
    outcomes.  A tuple is eligible only when the bounded transfer capture saw
    exactly one client SYN for it during the episode.  Sorting makes the wire
