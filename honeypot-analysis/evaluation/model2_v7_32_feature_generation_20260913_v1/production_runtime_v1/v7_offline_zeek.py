...
def run(
    *, request: Mapping[str, Any], capstone_pcap: bytes, transfer_ring_bases: list[pathlib.Path],
    ...
    if not episode_mode and (not isinstance(download_events, list) or len(download_events) != len(transfer_tuples)):
    ...
    with tempfile.TemporaryDirectory(prefix=request_id[:16] + "-", dir=work_dir) as temp_name:
        ...
        if transfer_tuples:
            ...
            if not episode_mode and not packet_set_matches_events(download_events, transfer_packets, transfer_tuples):
