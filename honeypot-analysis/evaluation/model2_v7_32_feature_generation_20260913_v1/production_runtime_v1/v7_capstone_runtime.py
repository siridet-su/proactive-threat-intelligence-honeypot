...
from model2_unified_session_candidate_20260926.production_adapter import (  # noqa: E402
    CAPTURE_CONTRACT as UNIFIED54_CAPTURE_CONTRACT,
    build_envelope as build_unified54_envelope,
)
from model2_unified_session_candidate_20260926.shadow_runtime import (  # noqa: E402
    infer_envelope as infer_unified54_shadow,
    load_candidate as load_unified54_candidate,
)
...
UNIFIED54_MANIFEST = UNIFIED54_ROOT / "CANDIDATE_MANIFEST.v1.json"


_v6_base_sanitized_event = v6.sanitized_event


def sanitized_v7_event(raw: Mapping[str, Any]) -> dict[str, Any]:
    """Retain only bounded, privacy-preserving 54F source projections."""
    event = _v6_base_sanitized_event(raw)
    username_sha256 = raw.get("username_sha256")
    if username_sha256 is not None:
        digest = str(username_sha256)
        if not re.fullmatch(r"[0-9a-f]{64}", digest):
            raise V7BoundaryError("username_projection_invalid")
        event["username_sha256"] = digest
    projection = raw.get("command_projection")
    if projection is not None:
        if not isinstance(projection, Mapping):
            raise V7BoundaryError("command_projection_invalid")
        encoded = canonical(dict(projection))
        if len(encoded) > 4096:
            raise V7BoundaryError("command_projection_too_large")
        allowed = {"family", "transfer"}
        if set(projection) - allowed:
            raise V7BoundaryError("command_projection_field_invalid")
        event["command_projection"] = json.loads(encoded)
    if event["eventid"] == "cowrie.session.file_download" and "download_request_sha256" in raw:
        digest = str(raw.get("download_request_sha256") or "")
        if not re.fullmatch(r"[0-9a-f]{64}", digest):
...
class Coordinator(v6.Coordinator):
    def __init__(self, config: Mapping[str, Any]) -> None:
        ...
        if self.shadow_enabled:
            ...
            self.unified54_model = load_unified54_candidate(
                UNIFIED54_ARTIFACT,
                feature_schema_path=UNIFIED54_SCHEMA,
                candidate_manifest_path=UNIFIED54_MANIFEST,
            )
    ...
    def _episode_tuples(self, raw_candidates: Any, contract: Any) -> list[dict[str, Any]]:
        ...
        candidates = [exact_tuple(value, "episode_packet_tuple") for value in raw_candidates]
        if any(not transfer_tuple_allowed(value) for value in candidates):
            raise V7BoundaryError("episode_packet_boundary_invalid")
        keys = [
            (value["src_ip"], value["src_port"], value["dst_ip"], value["dst_port"])
            for value in candidates
        ]
        if len(set(keys)) != len(keys) or keys != sorted(keys):
            raise V7BoundaryError("episode_packet_tuple_set_invalid")
        return candidates
    ...
    def _offline(
        ...
        expected = [exact_tuple(backend), *sensor_tuples, *episode_tuples]
        ...
        try:
            ...
            if self.shadow_enabled:
                ...
                candidate_result = infer_unified54_shadow(
                    unified54_envelope,
                    self.unified54_model,
                    expected_identity=identity,
                )
                if candidate_result.get("status") != "VALID_RESEARCH_SHADOW":
                    raise V7BoundaryError(
                        "unified54_inference_unavailable:" + str(candidate_result.get("reason") or "unknown")
                    )
                feature_gate_summary = candidate_result.get("feature_gate_summary")
                if not isinstance(feature_gate_summary, dict):
                    raise V7BoundaryError("unified54_feature_gate_summary_missing")
                transfer_tool_count = float(feature_gate_summary.get("transfer_tool_command_count", 0))
                t1105_transfer_observed = (
                ...
                for label, item in candidate_result["outputs"].items():
                    outputs[label] = {
                        ...
                        "raw_score": item["decision_score"],
                        "score_semantics": item["score_semantics"],
                        "threshold": item["threshold"],
                        "threshold_type": "sigmoid_decision_score",
                        "ensemble_vote_eligible": True,
                    }
                result = {
