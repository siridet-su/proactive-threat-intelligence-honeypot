"""Read-only Dashboard adapter for the retained Next-Distinct PoC.
...
def _read_latest_record(
    ...
    *,
    ...
    """Read the newest matching append-only JSONL record within fixed bounds.

    The feeder writes complete records with ``O_APPEND`` after a monotonic
    Mongo cursor, so reverse file order is the authoritative order for a
    given session.  A file-size check is deliberately not used here: the
    growing sidecar is searched from EOF in fixed chunks instead.
    """
    ...
    def finish(record: dict[str, Any] | None, error: str | None) -> tuple[dict[str, Any] | None, str | None]:
        if stats is not None:
            stats.update(
                {
                    "bytes_read": bytes_read,
                    "records_scanned": records_scanned,
                    "lookup_ms": round((time.monotonic() - started) * 1000.0, 3),
                }
            )
        return record, error

    if records_path.is_symlink() or not records_path.is_file():
        return finish(None, "sidecar records file is unavailable")
    try:
        file_size = records_path.stat().st_size
        with records_path.open("rb") as handle:
            position = file_size
            pending = b""
            while position > 0 and bytes_read < MAX_LOOKUP_BYTES:
                read_size = min(READ_CHUNK_BYTES, position, MAX_LOOKUP_BYTES - bytes_read)
                ...
                complete_parts = parts if position == 0 else parts[1:]
                for raw_line in reversed(complete_parts):
                    raw_line = raw_line[:-1] if raw_line.endswith(b"\r") else raw_line
                    if not raw_line:
                        continue
                    records_scanned += 1
                    if records_scanned > MAX_RECORDS_SCANNED:
                        return finish(None, "sidecar lookup exceeded safe record bound")
                    mentions_session = session_id.encode("utf-8") in raw_line
                    if len(raw_line) > MAX_RECORD_BYTES:
                        if mentions_session:
                            return finish(None, "no eligible sidecar record for this session: matching record exceeds maximum size")
                        continue
                    try:
                        ...
                        if mentions_session:
                            return finish(None, "no eligible sidecar record for this session: matching record is malformed")
                        continue
                    ...
                    try:
                        ...
                        return finish(None, "no eligible sidecar record for this session: matching record is invalid")
                    return finish(dict(value), None)
                pending = parts[0] if position > 0 else b""
            if position > 0:
                return finish(None, "sidecar lookup exceeded safe byte bound")
    except (OSError, UnicodeDecodeError) as exc:
        return finish(None, f"sidecar records file is unreadable: {type(exc).__name__}")
    return finish(None, "no eligible sidecar record for this session")
...
def build_dashboard_prediction(
    ...
    *,
    ...
    if not clean_id:
        return {"schema_version": EXPECTED_ADAPTER_SCHEMA, "dashboard_source": "NEXT_DISTINCT_POC", "prediction_status": "UNAVAILABLE", "prediction_status_reason": "session_id is required", "authority": "NON_AUTHORITATIVE_ADVISORY"}
    try:
