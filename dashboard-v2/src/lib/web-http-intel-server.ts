...
export async function getWebHttpHints(limit = 50): Promise<{ items: WebHttpHint[]; coverage: string }> {
  ...
  const rows = await client.db(getMongoDatabaseName()).collection("events")
    .find({ source: "web-corp", event_type: "web_login_attempt" }, {
      maxTimeMS: 5_000,
      ...
      projection: {
        ...
        "http.query": 1, "analysis.sqli.indicators": 1, outcome: 1,
      },
    })
  ...
  return {
    ...
    coverage: "Stored web-corp login attempts only; page views and bait paths are not in this durable lane.",
  };
}
