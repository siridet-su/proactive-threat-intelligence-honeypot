      ...
      <AnimatePresence initial={false}>
        {open && (
          <motion.div
            ...
            className="absolute right-0 top-[calc(100%+8px)] z-[80] w-[min(92vw,360px)] overflow-hidden rounded-2xl border border-border bg-surface-raised shadow-2xl"
          >
          <div className="flex items-center justify-between border-b border-border/60 bg-surface-subtle/50 px-4 py-3">
            <div>
              <div className="flex items-center gap-2 text-xs font-semibold text-text">
                <CalendarDays className="h-3.5 w-3.5 text-primary" aria-hidden="true" />
                Custom history range
              </div>
              <p className="mt-0.5 text-[11px] text-text-subtle">Select up to 30 days of retained history</p>
            </div>
            <button type="button" onClick={() => setOpen(false)} className="rounded-md p-1.5 text-text-subtle hover:bg-surface-hover hover:text-text" aria-label="Close custom history range">
              <X className="h-4 w-4" aria-hidden="true" />
            </button>
          </div>

          <div className="flex justify-center border-b border-border/60 px-2 py-1">
            <Calendar
              mode="range"
          ...
          <div className="grid gap-2 border-b border-border/60 bg-surface-subtle/30 p-3">
            ...
            />
            {validationError && <p className="text-xs text-warning">{validationError}</p>}
          </div>

          <div className="flex items-center justify-end gap-2 bg-surface-subtle/50 p-3">
            <button type="button" onClick={() => setOpen(false)} className="ui-button min-h-9 px-3 text-xs">
              Cancel
            </button>
            <button type="button" onClick={handleApply} disabled={Boolean(validationError)} className="ui-button min-h-9 gap-1.5 border-primary-border bg-primary-subtle px-3 text-xs text-primary disabled:cursor-not-allowed disabled:opacity-50">
