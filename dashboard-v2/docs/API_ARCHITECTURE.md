...
The default BFF origin is `http://127.0.0.1:8090`, the existing `monitor_web` service. `monitor_web` supplies liveness/readiness, the structured session/event/advisory routes, and the semantic/table routes. `dashboard_api` supplies the semantic/table routes and overlaps the health routes, but it does not implement the monitor-specific `/api/sessions`, `/api/session`, `/api/events`, or `/api/ai-advisory` paths. A deployment that changes `DASHBOARD_API_ORIGIN` must preserve this compatibility or those paths will return an upstream not-found response.
