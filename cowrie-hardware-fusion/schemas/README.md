# Schemas

งานแรกของโฟลเดอร์นี้คือกำหนดสัญญาต่อไปนี้ก่อนเก็บ full dataset:

- experiment/run manifest
- raw telemetry sample
- Cowrie command-to-run correlation
- impact labels และ ground-truth TTP set
- derived training window
- shadow prediction record

ทุก schema ต้องแยก metric scope เช่น `pi_sensor`, `backend_guest`, `backend_cgroup`
และ `target_process` เพื่อไม่รวมค่าจากคนละเครื่องหรือคนละ authority boundary

ไฟล์ปัจจุบัน:

- [Experiment run manifest schema](experiment_run_manifest.v1.schema.json)
- [Hardware telemetry sample schema](hardware_telemetry_sample.v1.schema.json)
- [Derived training window schema](derived_training_window.v1.schema.json)
- [Derived training window schema v2](derived_training_window.v2.schema.json)
- [Experimental collector config schema](experimental_collector_config.v1.schema.json)
- [Experiment collection receipt schema](experiment_collection_receipt.v1.schema.json)
- [Dataset source index schema](dataset_source_index.v1.schema.json)
- [Dataset split assignment schema](dataset_split_assignment.v1.schema.json)
- [Bounded workload specification schema](bounded_workload_spec.v1.schema.json)
- [Bounded workload preflight receipt schema](bounded_workload_preflight_receipt.v1.schema.json)
- [Pi PoC workload specification schema](pi_poc_workload_spec.v1.schema.json)
- [Pi PoC execution receipt schema](pi_poc_execution_receipt.v1.schema.json)
- [Pi PoC matrix schema](pi_poc_matrix.v1.schema.json)
- [Hardware-impact experiment protocol v2 schema](hardware_impact_experiment_protocol.v2.schema.json)
- [Service-pressure instrumentation matrix schema v1](service_pressure_instrumentation_matrix.v1.schema.json)
- [Service-pressure workload specification schema v1](service_pressure_workload_spec.v1.schema.json)
- [Service-pressure instrumentation matrix schema v2](service_pressure_instrumentation_matrix.v2.schema.json)
- [Service-pressure workload specification schema v2](service_pressure_workload_spec.v2.schema.json)
- [Service-pressure candidate signal report schema](service_pressure_signal_report.v1.schema.json)
- [Model feature contract schema](model_feature_contract.v1.schema.json)
- [Example run manifest](examples/experiment_run_manifest.v1.example.json)
- [Example bounded benign-compute manifest](examples/experiment_run_manifest.benign_compute.v1.example.json)
- [Example telemetry sample](examples/hardware_telemetry_sample.v1.example.json)
