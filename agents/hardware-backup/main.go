...
func run() error {
	...
	latestDay := today.Add(-time.Duration(cfg.SafetyDays) * 24 * time.Hour)
	oldestDay := today.Add(-time.Duration(cfg.LookbackDays) * 24 * time.Hour)
	log.Printf(
	...
	var failures []error
	for day := oldestDay; !day.After(latestDay); day = day.Add(24 * time.Hour) {
		if err := backupDay(ctx, collection, manifests, b2, cfg, day); err != nil {
